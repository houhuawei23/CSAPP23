...
#define GHR_LEN 10 // 全局历史寄存器长度，取10位
...
void PREDICTOR_init(void)
{
    ...
    ghr = 0; // Initialize global history register
}
...
char GetPrediction(UINT64 PC)
{
    ...
    UINT32 combinedHistory = (localHistory ^ ghr) % numPhtEntries;
    UINT32 phtCounter = pht[combinedHistory];
...
void UpdatePredictor(UINT64 PC, OpType opType, char resolveDir, char predDir, UINT64 branchTarget)
{
    ...
    if (resolveDir == TAKEN)
    {
        pht[combinedHistory] = SatIncrement(phtCounter, PHT_CTR_MAX);
    }
    else
    {
        pht[combinedHistory] = SatDecrement(phtCounter);
    }

    // Update local history table
    localHistory = (localHistory << 1) & ((1 << LHT_LEN) - 1);
